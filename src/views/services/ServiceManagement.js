...
const API_URL = 'http://44.196.64.110/api/service/getAllServices';
...
const ServiceManagement = () => {
  ...
  const handleAddService = async () => {
    ...
    try {
      await axios.post('http://44.196.64.110/api/service/createService', payload);
      fetchServices();
  ...
  const handleSaveEditService = async () => {
    try {
      await axios.put(`http://44.196.64.110/api/service/editService/${editService._id}`, editService);
      fetchServices();
  ...
  const handleDeleteService = async (serviceId) => {
    if (window.confirm("Are you sure you want to delete this service?")) {
      try {
        await axios.delete(`http://44.196.64.110/api/service/delete/${serviceId}`);
        fetchServices();
      } catch (error) {
