...
const JobsManagement = () => {
    ...
    try {
      ...
      if (jobStatusFilter !== 'All') {
        response = await axios.get(
          `http://18.209.91.97:7777/api/jobs/jobPostAdmin?status=${jobStatusFilter}&page=${page}&limit=10`,
          commonConfig,
        )
      } else {
        response = await axios.get(
          `http://18.209.91.97:7777/api/jobs/jobPostAdmin?page=${page}&limit=10&search=${search}`,
          commonConfig,
        )
      }
  ...
  const handleDeleteJob = async (jobId) => {
    if (window.confirm('Are you sure you want to delete this job?')) {
      try {
        await axios.delete(`http://18.209.91.97:7777/api/jobPost/${jobId}`, commonConfig)
        fetchJobs()
      } catch (error) {
  ...
  const handleSaveEditJob = async () => {
    try {
      ...
      await axios.put(
        `http://18.209.91.97:7777/api/jobpost/${editJob._id}`,
        updatedJob,
        commonConfig,
      )
