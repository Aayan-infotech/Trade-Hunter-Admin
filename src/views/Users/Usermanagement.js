...
const Usermanagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://18.209.91.97:7787/api/users');
  ...
  const deleteUser = async (id) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        await axios.delete(`http://18.209.91.97:7787/api/users/${id}`);
  ...
  const handleSave = async () => {
    try {
      await axios.put(`http://18.209.91.97:7787/api/users/${selectedUser._id}`, selectedUser);
      fetchUsers();
      setShowModal(false);
    } catch (error) {
  ...
  const renderFilePreview = (file) => {
    ...
    if (['jpg', 'png', 'jpeg'].includes(fileExtension)) {
      return <img src={`http://18.209.91.97:7787/uploads/${file}`} alt={file} style={{ width: '100px', height: 'auto' }} />;
    }
    if (fileExtension === 'pdf') {
      return <embed src={`http://18.209.91.97:7787/uploads/${file}`} type="application/pdf" width="100px" height="100px" />;
    }
    return <a href={`http://18.209.91.97:7787/uploads/${file}`} target="_blank" rel="noopener noreferrer">Download {file}</a>;
  };
