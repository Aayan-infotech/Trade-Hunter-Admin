...
const Usermanagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://54.236.98.193:7777/api/users');
  ...
  const handleSave = async () => {
    try {
      await axios.put(`http://54.236.98.193:7777/api/users/${selectedUser._id}`, selectedUser);
      fetchUsers();
      setShowModal(false);
    } catch (error) {
  ...
  const renderFilePreview = (file) => {
    ...
    if (['jpg', 'png', 'jpeg'].includes(fileExtension)) {
      return <img src={`http://54.236.98.193:7777/uploads/${file}`} alt={file} style={{ width: '100px', height: 'auto' }} />;
    }
    if (fileExtension === 'pdf') {
      return <embed src={`http://54.236.98.193:7777/uploads/${file}`} type="application/pdf" width="100px" height="100px" />;
    }
    return <a href={`http://54.236.98.193:7777/uploads/${file}`} target="_blank" rel="noopener noreferrer">Download {file}</a>;
  };
