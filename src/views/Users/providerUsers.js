...
const Provider = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get(
        `http://3.223.253.106:7777/api/Prvdr?page=${page}&limit=10&search=${search}&userStatus=${statusFilter}`,authHeaders
      )
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        await axios.delete(`http://3.223.253.106:7777/api/prvdr/delete/${id}`,authHeaders)
        fetchUsers()
      } catch (error) {
  ...
  const handleSaveEdit = async () => {
    try {
      await axios.put(`http://3.223.253.106:7777/api/Prvdr/${editUser._id}`,authHeaders, editUser)
      fetchUsers()
  ...
  const fetchNotifications = async (userId) => {
    try {
      const response = await axios.get(
        `http://3.223.253.106:7777/api/pushNotification/getAdminNotification/${userId}`,authHeaders
      )
  ...
  const handleSendNotification = async () => {
    ...
    try {
      await axios.post(
        `http://3.223.253.106:7777/api/pushNotification/sendAdminNotification/${notifUser._id}`,authHeaders,
        {
          title: notifTitle,
          body: notifBody,
        },
  ...
  const handleDeleteNotification = async (notifId) => {
    ...
    const deleteUrl = (`http://3.223.253.106:7777/api/pushNotification/deleteNotification/${notifId}`,authHeaders)
