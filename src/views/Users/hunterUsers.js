...
const Hunter = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await axios.get(
        `https://api.tradehunters.com.au/api/users/type/hunter/pagelimit/10?page=${page}&search=${search}&userStatus=${statusFilter}`,
        authHeaders
      )
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        await axios.delete(
          `https://api.tradehunters.com.au/api/DeleteAccount/hunter/${id}`,
          authHeaders
        )
        fetchUsers()
      } catch (error) {
  ...
  const handleSaveEdit = async () => {
    try {
      await axios.put(
        `https://api.tradehunters.com.au/api/users/${editUser._id}`,
        editUser,
        authHeaders
      )
      fetchUsers()
  ...
  const handleSendNotification = async () => {
    ...
    try {
      await axios.post(
        ...
        { title: notifTitle, body: "Notification from  Trade Hunters Admin Team" + '--' + notifBody },
        authHeaders
      )
      await axios.post(`https://api.tradehunters.com.au/api/hunter/sendSupportEmail`, {
        name: notifUser.name,
        email: notifUser.email,
        message: `Notification from  Trade Hunters Admin Team: ${notifBody}`,
