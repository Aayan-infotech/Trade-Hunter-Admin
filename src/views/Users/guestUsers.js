...
const GuestUsers = () => {
  ...
  const fetchGuestUsers = async () => {
    ...
    try {
      const response = await axios.get(
        `https://api.tradehunters.com.au/api/Prvdr/GuestMode/?page=${page}&limit=10&search=${search}`,
        authHeaders,
      )
    ...
    try {
      await axios.post(
        `https://api.tradehunters.com.au/api/pushNotification/sendAdminNotification/${notifUser._id}`,
        {
          title: notifTitle,
          body:
            'Notification from  Trade Hunters Admin Team -- ' +
            notifBody,
        },
        authHeaders,
      )
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm('Are you sure you want to delete this guest user?')) {
      try {
        await axios.delete(
          `https://api.tradehunters.com.au/api/DeleteAccount/provider/${userId}`,
          authHeaders,
        )
